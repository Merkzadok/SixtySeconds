  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-gray-600">Crack the 4-color code in 8 attempts or less!</p>
      </div>
        ...
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4 text-center">
          ...
          <div className="flex justify-center gap-1 mt-2">
            {secretCode.map((colorIndex, i) => (
              <div key={i} className={`w-6 h-6 rounded-full border-2 border-gray-300 ${COLORS[colorIndex].bg}`} />
            ))}
          </div>
        </div>
      )}
      ...
      {/* Previous Guesses */}
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-700 mb-3">Attempts ({guesses.length}/8)</h3>
        <div className="space-y-2">
          {guesses.map((guess, guessIndex) => (
            <div key={guessIndex} className="flex items-center gap-3 p-2 bg-gray-50 rounded">
              {/* Guess colors */}
              <div className="flex gap-1">
                {guess.map((colorIndex, i) => (
                  <div key={i} className={`w-8 h-8 rounded-full border-2 border-gray-300 ${COLORS[colorIndex].bg}`} />
                ))}
              </div>
              ...
              <div className="flex gap-1 ml-auto">
                {Array.from({ length: feedback[guessIndex].black }, (_, i) => (
                  <div key={`black-${i}`} className="w-3 h-3 bg-black rounded-full" />
                ))}
                {Array.from({ length: feedback[guessIndex].white }, (_, i) => (
                  <div key={`white-${i}`} className="w-3 h-3 bg-white border border-gray-400 rounded-full" />
                ))}
              </div>
            </div>
          ))}
      ...
      {gameState === "playing" && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-700 mb-3">Current Guess</h3>
            ...
            {Array.from({ length: 4 }, (_, i) => (
              <button
                ...
                className={`w-12 h-12 rounded-full border-4 transition-all ${
                  selectedPosition === i ? "border-blue-500 shadow-lg" : "border-gray-300 hover:border-gray-400"
                } ${currentGuess[i] !== undefined ? COLORS[currentGuess[i]].bg : "bg-gray-100"}`}
              />
            ))}
          </div>
      ...
      {/* Restart Button */}
      <Button onClick={initializeGame} variant="outline" className="w-full bg-transparent">
        New Game
      </Button>
